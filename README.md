...
Version 0.1.4 corrects capacity accounting for occupied filtered requester slots, backs off failed temporary inserter stack-size restorations, and removes unused saved-state bookkeeping. Batch modes and READY/COMPLETE handshake semantics are unchanged.
...
Planning uses current matching contents, requester/filter capacity, captured effective pickup count where required, and item stack-size limits. An occupied filtered slot contributes free capacity for its contents only when its exact item-and-quality filter matches those contents. Managed inventory filters support exact `=` item-quality identities only; non-exact quality comparators fail closed before requests activate. Single never falls back to another mode. Its bounded search examines balance, constraint, transfer-congruence, and inventory-slot breakpoints rather than requested units; if a safe allocation is found before later optimality proof reaches the bound, that validated allocation is retained, while exhaustion without a safe allocation reports a distinct visible limit instead of claiming impossibility.
...
To limit UPS impact, combinator checks are spread across game ticks instead of running all at once. Exact requesters use Factorio's native exact behavior; compatible non-exact requesters use direct logistic-point and inventory reads without scanning robots. Empty unfiltered large inventories take a constant-time capacity fast path, No-tail avoids inserter scans while READY source chests still contain items, and failed temporary inserter stack-size restorations wait 60 ticks between native write attempts. Automatic-cleanup work remains idle until cleanup starts, and the detailed window is not continually refreshed while closed. Large-factory UPS gains have not been measured.
...
The gameplay code prepared for 0.1.4 passed the offline regression suite and automated Factorio 2.1.17 checks covering 17 runtime cases and all six Debug Lab outcome cases. It was also checked manually through FMTK in English at 1920×1080 and 100% UI scale, with no issues reported and a clean session log.

This remains experimental. Stateful save/reload and upgrade recovery, native robot-overdelivery edge cases, multiplayer, additional languages and UI scales, and matched large-factory performance remain unverified for this version. The FMTK empty-lab fixture does not support reloading a populated test world; save/reload proof requires a separate suitable fixture.
